...
# 선형리스트 [ Linear List ]
# 연결리스트 [ 1.싱글 연결리스트( Signly Linked List)  2.원형 연결리스트 ]
# 선형 vs 연결
    # 1. 순서대로  vs 연결식
    # 2. 인덱스 사용 v 노드 사용
        # 인덱스 형식   :   [ 1 , 2 , 3 , 4 ]
            # 1. 인덱스( 저장되는 순서 ) 이용한 검색 속도 빠름
            # 2. 추가 빠름
        # 노드 형식     :   1  -->   2  -->  3 --> 4
            # 2. 연결형식 : 중간 삽입/삭제 빠름
...
# 1. 노드 클래스( 데이터, 링크 )  선언
class Node( ) : # 1. Node 라는 이름으로 클래스 선언
...
print( node1.link.link.link.link.data ) # 헤더의 링크 -> 링크 -> 링크 -> 링크  출력
...
def node_print( ) :
    ...
    while current.link !=None : # 링크가 없을때 까지 반복
...
newnode.data = '하하'
    ...
    #           newnode : 하하
    #
newnode.link = node2.link       # 강호동이 가지고 있던 신동엽 주소(링크)를 하하에게 대입
node2.link = newnode            # 강호동은 하하의 주소를 가진다.
